...
def test_from_string(har_data):
    with open(har_data("humanssuck.net.har", True), encoding="utf-8") as infile:
        data = infile.read()
...
def test_create_asset_timeline(har_data):
    ...
    for _ in range(1, load_time):
        ...
        # Compare the dicts
        for _, _ in asset_timeline.items():
            assert (
                du.parse(asset_timeline[time_key][0].raw_entry["startedDateTime"])
                == entry.startTime
            )
...
def _headers_test(parser, entry, test_data, expects, regex):
    ...
    for req_type, data in test_data.items():
        for header, value in data.items():
            is_match = parser.match_headers(entry, req_type, header, value, regex=regex)
            if expects:
                assert is_match
            else:
                assert not is_match
